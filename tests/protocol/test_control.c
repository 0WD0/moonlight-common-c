...
static bool test_request_round_trip_bounds(void) {
  ...
  TEST_CHECK(encoded[18] == 0 && encoded[24] == 0x07 && encoded[25] == 0xff);
...
static bool test_request_encode_rejections(void) {
  ...
  request.capability_bits = UINT64_C(0x800);  // Undefined capability bit (bit 11).
  TEST_RESULT(
    MoonlightProtocolV1EncodeClientHelloRequest(
      &request,
      output,
...
static bool test_response_encode_rejections(void) {
  ...
  response.capability_bits = UINT64_C(0x800);  // Undefined capability bit (bit 11).
  TEST_RESULT(
    MoonlightProtocolV1EncodeClientHelloResponse(
      &response,
      output,
